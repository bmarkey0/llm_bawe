...
cond1 = """
You're a teacher grading a paper. Use a five point scale to grade whether the following paper uses correct grammar. 
Do not summarize or provide feedback. Just give the grade (e.g, Level 2, Level 4, etc.)
where level 1 is the least grammatically correct, and 5 is the most grammatically correct. 
"""

cond2 = """
You're a teacher grading a paper. Use a five point scale to grade whether the following paper uses correct grammar. 
Do not summarize or provide feedback. Just give the grade (e.g., Level 2, Level 4, etc.) 
where level 1 is the most grammatically incorrect, and 5 is the most grammatically correct. 
"""

cond3 = """
You're a teacher grading a paper. Use a five point scale to grade whether the following paper uses correct grammar. 
Do not summarize or provide feedback. Just give the grade (e.g., level 2, level 4).  Use the following five point 
scale, where level 5 is the highest and level 1 is the lowest:

5: Skillful use of varied sentence structure contributes to fluidity of ideas. Use of standard English grammar, 
punctuation, capitalization, and spelling demonstrates consistent command of the communication of ideas.

4: Sentence structure is varied and demonstrates language facility. Use of grammar, punctuation, capitalization, 
and spelling demonstrates appropriate command of standard English conventions.

3: Sentence structure is controlled, though somewhat simplistic. Inconsistent use of correct grammar, punctuation, 
capitalization, and/or spelling contains a few distracting errors, demonstrating inconsistent command of standard 
English conventions and the clear communication of ideas inconsistent. 

2: Sentence structure is partially controlled, somewhat simplistic, or lacking appropriate language facility. 
Inconsistent use of correct grammar, punctuation, capitalization, and/or spelling contain multiple distracting 
errors, demonstrating partial command of standard English conventions.

1: Sentence structure is simplistic or confusing. Use of grammar, punctuation, capitalization, and/or 
spelling evidences a density and variety of severe errors, demonstrating lack of command of standard English 
conventions, often obscuring meaning.

"""

cond4 = """
You're a teacher grading a paper. Use a five point scale to grade whether the following paper uses correct grammar. 
Do not summarize or provide feedback. Just give the grade (e.g., level 2, level 4, etc.). Use a five point 
scale, where level 5 is the highest and level 1 is the lowest. Here are descriptions of levels 5, 3, and 1:  
...
cond6 = """
You're a teacher grading a paper. Use a five point scale to grade whether the following paper uses correct grammar. Do 
not summarize or provide feedback. Just give the grade (e.g., level 2, level 4, etc.). Use the following five point 
...
def get_completions(texts, model, temp, tensor_parallel_size=2):
    sampling_params = SamplingParams(logprobs=3, top_p=0.9,
                                     max_tokens=10, repetition_penalty=1.0,
                                     temperature=temp)
    ...
    print(f"Prepared system prompt: {cond_list[args.condition]}")
...
if __name__ == "__main__":
    ...
    for item in temp_lst:
        ...
        pq.write_table(out_table, os.path.join(args.outdir, model_name + f"#Essay_grammar_{item}.parquet"), compression="gzip")
